...
func main() {
	...
	if zigodb.Global().NeedsDrain() {
		...
		if err != nil {
			fmt.Printf("Drain error (expected - not fully implemented): %v\n", err)
		}
	}
	...
	// Export chunk to .rb file (T4.7)
	// Note: This is not fully implemented in Zig yet
	filename := "storage/chunks/chat_001.rb"
	err = zigodb.Global().ExportChunk(filename)
	if err != nil {
